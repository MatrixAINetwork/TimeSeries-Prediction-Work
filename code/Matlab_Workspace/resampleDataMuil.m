function resampleDataMuil
   %导入数据-------------------------------------------------------------------
    ...
    %filepath = 'E:\\Data_Lab\\20170208-tcms-cc66666-240-分天数据-所有11条\\original\\拆分后合并文件';
    filepath = 'E:\\Data_Lab\\数据驱动\\out\\3.remove';
    filePaths = getFilelist(filepath);  
    for i = 1:length(filePaths)
        resampleData2(filePaths{i},10);
    end    
end
...
%数据处理-------------------------------------------------------------------
    ...
    filename_out = filename_in(1:end-7);
    filename_out = strcat(filename_out, '_ip_', num2str(interval), '.csv');
