...
func.var<-function(data, sampleEndIndex, prdRange, col){
  ...
  varprd<-predict(varest,n.ahead = prdRange,ci=0.95);
...
#同func.var
#1.修改tmp.error为tmp.errorRatio;修改tmp.maxError为tmp.maxErrorRatio
#  修改colvalue[["error"]]为colvalue[["errorRatio"]];修改colvalue[["maxError"]]为colvalue[["maxErrorRatio"]]
#2.添加colvalue[["error"]]
#3.删除画图代码
func.var2<-function(data, sampleEndIndex, prdRange, col){
  ...
  if(col == 0){
    ...
    for(col in 1:6){
      ...
      tmp.ori.prd.col<-data[(sampleEndIndex+1):(sampleEndIndex+prdRange),col];#get the original data to the corresbonding predic value for the pointed col
      tmp.prd.col<-varprd$fcst[[col]][,1];#get the predicted value of pointed column
      tmp.errorRatio<-abs(tmp.ori.prd.col-tmp.prd.col)/tmp.prd.col*100;#count the prediction error
      tmp.maxErrorRatio<-max(tmp.errorRatio)
      tmp.error<-(tmp.prd.col-tmp.ori.prd.col)
      
      #save the data
      colvalue[["ori"]]<-tmp.ori.prd.col
      colvalue[["prd"]]<-tmp.prd.col
      colvalue[["errorRatio"]]<-tmp.errorRatio
      colvalue[["error"]]<-tmp.error
      colvalue[["maxErrorRatio"]]<-tmp.maxErrorRatio      
      colist[[col]]<-colvalue
    }
...
#分析跳跃剧烈的数据
func.var.jump<-function(allAxisTemp.merge){
  ...
  endLen<-800
  len<-200
  res<-func.var(axleList[[axle]], endLen, len, point)
